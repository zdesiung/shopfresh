...
export default function YapeQR() {
  ...
  const handleConfirmPayment = () => {
    ...
    setTimeout(() => {
      setLoading(false);
      setConfirmed(true);
    }, 2500); // Simula confirmación del pago
  };
  ...
  return (
    ...
    >
      <h2 className="text-xl font-semibold text-zinc-900 dark:text-white flex items-center justify-center gap-2 mb-3">
        <Smartphone className="text-purple-600 w-5 h-5" /> Pago con Yape
      </h2>

      {!confirmed ? (
        <>
          <p className="text-zinc-600 dark:text-zinc-300 mb-4 text-sm">
            Escanea este código QR con tu aplicación <b>Yape</b> para realizar
            el pago.
          </p>
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.97 }}
            disabled={loading}
            className="bg-purple-600 hover:bg-purple-700 transition text-white font-semibold py-2 px-5 rounded-lg flex items-center justify-center gap-2 mx-auto"
          >
            {loading ? (
              <>
        ...
        >
          ...
          <p className="text-sm text-zinc-500 dark:text-zinc-400 mt-1">
            Gracias por tu compra con <b>Yape</b>.
          </p>
        </motion.div>
      )}
    </motion.div>
